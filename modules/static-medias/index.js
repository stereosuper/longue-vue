        ...
        fileStream.on('finish', () => {
            ...
            if (filePath.match(imageRegex)) {
                ...
                sharp(filePath)
                    ...
                    .then(() => {
                        logger.success(
                            'Successfully downloaded and converted to webp!',
                        );
                        resolve();
                    })
                    .catch(err => {
    ...
    this.nuxt.hook('generate:done', async () => {
        ...
        const promisesUrls = routes.map(async route => {
            ...
            const promisesPayloadFiles = payloadFiles.map(async payloadFile => {
                ...
                let payload = await readFile(payloadPath, 'utf-8').then(res =>
                    res.replace(/%2F/g, '/'),
                );
                ...
                if (images) {
                    mediasUrls.images = [
                        ...new Set([...mediasUrls.images, ...images]),
                    ];
                ...
                if (tracks) {
                    mediasUrls.tracks = [
                        ...new Set([...mediasUrls.tracks, ...tracks]),
                    ];
                    payload = replaceUrlsInJson({
                        array: tracks,
                        json: payload,
        ...
        const handleFilesTypes = async ([type, urls]) => {
            ...
            logger.info(
                `Downloading ${type.charAt(0).toUpperCase() + type.slice(1)}`,
            );
